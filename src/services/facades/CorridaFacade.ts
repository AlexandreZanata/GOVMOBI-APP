...
export class CorridaFacadeImpl implements ICorridaFacade {
  ...
  public async getActiveCorrida(): Promise<Result<Corrida | null, FacadeError>> {
    try {
      ...
      if (!response.ok) return fail(toError('Unable to fetch active ride', 'NETWORK_ERROR'));
      const data = (await response.json()) as Corrida;
      return ok(data);
    } catch {
      return fail(toError('Network error while fetching active ride', 'NETWORK_ERROR'));
    }
  }
