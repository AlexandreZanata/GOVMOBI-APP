...
// ---------------------------------------------------------------------------
// First-install / post-logout scenario — previousStatus === null
// ---------------------------------------------------------------------------

describe('useAuthSession — first install / post-logout: null previousStatus triggers restore', () => {
  /**
   * Root cause of "driver never receives rides" bug:
   *
   * On first install or after logout, statusOperacional is null in Redux.
   * doGetMe sets it to OFFLINE (server auto-sets on WS disconnect).
   * The old restore condition only checked previousStatus === 'DISPONIVEL',
   * so null was never restored — leaving the driver stuck in OFFLINE.
   *
   * Fix: previousStatus === null must also trigger the restore, because null
   * means the driver never explicitly went offline.
   */
  it('restores DISPONIVEL when previousStatus=null and server returns OFFLINE', async () => {
    mockGetMe.mockResolvedValue({
      data: {...MOCK_ME_BASE, statusOperacional: 'OFFLINE'},
      error: null,
    });

    // null = no persisted status (first install or post-logout)
  ...
  it('restores DISPONIVEL when previousStatus=null and server returns INDISPONIVEL', async () => {
    mockGetMe.mockResolvedValue({
      data: {...MOCK_ME_BASE, statusOperacional: 'INDISPONIVEL'},
      error: null,
    });
  ...
  it('does NOT call PATCH when previousStatus=null and server already returns DISPONIVEL', async () => {
    mockGetMe.mockResolvedValue({
      data: {...MOCK_ME_BASE, statusOperacional: 'DISPONIVEL'},
      error: null,
    });
    ...
    // Server already has DISPONIVEL — no PATCH needed
    expect(mockUpdateMyStatus).not.toHaveBeenCalled();

    unmount();
  });

  it('optimistically sets DISPONIVEL in Redux even when PATCH fails', async () => {
    mockGetMe.mockResolvedValue({
      data: {...MOCK_ME_BASE, statusOperacional: 'OFFLINE'},
      error: null,
    });
    // Simulate PATCH failure
    mockUpdateMyStatus.mockResolvedValue({data: null, error: {code: 'NETWORK_ERROR', message: 'Network error'}});
    ...
    // Even on PATCH failure, Redux must be set to DISPONIVEL so the location
    // stream can emit ficar-disponivel and the driver can receive rides.
    expect(store.getState().auth.statusOperacional).toBe('DISPONIVEL');
