 ...
 * Shows a fixed driver/vehicle card above the route addresses, status, cancel, and chat FAB.
 */
...
const DRIVER_ASSIGNED = new Set(['aceita', 'em_rota', 'passageiro_a_bordo']);
const TERMINAL_STATUSES = new Set(['concluida', 'cancelada', 'expirada', 'avaliada']);
const FAB_GAP = 16;

/**
 * Active ride panel for the passenger home screen.
 * Driver name + vehicle are shown in a fixed strip above the route addresses.
 *
  ...
  return (
    <>
      <View
        ...
        {/* Fixed driver + vehicle strip — above the route addresses */}
        {showDriverStrip && (
          <View style={styles.driverStrip} testID="driver-strip">
            ...
            <View style={styles.driverStripInfo}>
              ...
              {veiculoLabel ? (
                <Text style={styles.driverStripVehicle} numberOfLines={1}>
                  {veiculoLabel}
                </Text>
              ) : null}
            </View>
        ...
        {showDriverStrip && <View style={styles.driverStripDivider} />}
      ...
      {/* Chat FAB — positioned dynamically above the sheet */}
      {!isTerminal && (
        <TouchableOpacity
